package rogueutil.fov 
{
	...
	public class FOV 
	{
		...
		/**
		 * Call back these functions with the x,y of tiles in light or darkness
		 */
		public function callOnVisible(cbVisible:Function, cbDark:Function = null):void {
			if(!_fovMap){return}
			for (var y:int = -_radiusI; y <= _radiusI; y++) {
				for (var x:int = -_radiusI; x <= _radiusI; x++) {
					if (getLocalVisible(x, y)) {
						cbVisible(x + _ax_cb, y + _ay_cb)
					}else {
						if(cbDark != null){cbDark(x + _ax_cb, y + _ay_cb)}
					}
				}
			}
